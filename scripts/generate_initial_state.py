...
def main() -> None:
    ...
    parser.add_argument("--pattern", type=str, default="glider", help="Pattern to place, or 'random'")
    ...
    if args.pattern == "random":
        grid = np.random.choice([0, 1], size=(args.rows, args.cols), p=[1 - args.density, args.density]).astype(np.uint8)
    else:
