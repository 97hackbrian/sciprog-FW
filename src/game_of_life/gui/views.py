...
class GridView:
    ...
    def __init__(self, parent: int | str, rows: int, cols: int):
        ...
        with dpg.plot(label="Game of Life Grid", width=-1, height=-1, parent=parent,
                      equal_aspects=True, no_menus=True, no_title=True) as self.plot_id:
            
            self.x_axis = dpg.add_plot_axis(dpg.mvXAxis, no_gridlines=True, no_tick_marks=True, no_tick_labels=True)
            self.y_axis = dpg.add_plot_axis(dpg.mvYAxis, no_gridlines=True, no_tick_marks=True, no_tick_labels=True)
            
            # The heat series expects a flat list of floats, normalized between scale_min and scale_max
            # Note: Dear PyGui heat series takes data in column-major order or row-major depending on version,
            # but usually it's just a flattened 1D list of the 2D array.
            dummy_data = [0.0] * (rows * cols)
            self.series_id = dpg.add_heat_series(
                dummy_data, rows=rows, cols=cols, scale_min=0, scale_max=1,
                parent=self.y_axis, format=""
            )
            
            # Use a custom colormap (Black for 0, Green for 1)
            with dpg.colormapRegistry():
