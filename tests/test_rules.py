...
from game_of_life.patterns.catalog import CATALOG, PatternCategory
...
def run_pattern_for_period(cells: frozenset[tuple[int, int]], period: int, expected_cells: int, 
                           check_translation: bool = False) -> None:
    ...
    result_cells = {(r, c) for r in range(grid_size) for c in range(grid_size) if engine.grid.array[r, c] == 1}
...
def test_still_lifes(pattern_name: str) -> None:
    ...
    run_pattern_for_period(pattern.cells, period=1, expected_cells=len(pattern.cells), check_translation=False)
...
def test_oscillators(pattern_name: str) -> None:
    ...
    run_pattern_for_period(pattern.cells, period=pattern.period, expected_cells=len(pattern.cells), check_translation=False)
...
def test_spaceships(pattern_name: str) -> None:
    ...
    run_pattern_for_period(pattern.cells, period=pattern.period, expected_cells=len(pattern.cells), check_translation=True)
